...
Odd-n unpaired extra at j=2n-5U-4 has count J_k+(-1)^k for k>=2.
The first cell is Cycle WP's n=5U/2+5, j=6 for k>=4. Dies at k=2
for equals J_{k+1} (got 2, not 3). Dies at k=7 for equals J_k
(got 42, not 43). Dies at k=8 for equals clip_gp (got 86, not 85).
Do not kill equals clip_gp at k=7 (both 42). Do not kill pal_kind
unpaired at n=5U/2+5, j=6 for k>=4. Do not PREFIX pal-center tot.
...
Run: python3 research/cycle_wu.py --certify
Dump: research/cycle_wu.json
"""
...
def want_odd_s4(k: int) -> int:
    """Odd-n unpaired extra at partner 5U+4: 0 at k<=1, J_k+(-1)^k for k>=2."""
    if k <= 1:
        return 0
    return jacobsthal(k) + ((-1) ** k)


def odd_s4_split(k: int) -> dict:
    """Odd-n unpaired extra at j=2n-5U-4. Do not call from tot_form."""
    u = 1 << k
    clip = 5 * u
    n_at = n_bad = 0
    first = None
...
def tot_form() -> dict:
    """k<=64: odd slice J_k+(-1)^k; dies at k=2 for J_{k+1}.

    Do not call odd_s4_split here.
    Census is odd_s4_fold for k<=8.
    """
    n_ok = 0
    if want_odd_s4(0) != 0 or want_odd_s4(1) != 0:
        return {"ok": False, "k01": True}
    if want_odd_s4(2) != 2 or want_odd_s4(3) != 2:
        return {"ok": False, "k23": True}
    if want_odd_s4(8) != 86 or want_odd_s4(7) != 42:
        return {"ok": False, "k78": True}
    if want_odd_s4(7) != want_clip_gp(7):
    ...
    for k in range(0, K_ALG + 1):
        ...
        if k <= 1 and want_odd_s4(k) != 0:
            return {"ok": False, "z": True, "k": k}
        if k >= 2:
            if want_odd_s4(k) != jacobsthal(k) + ((-1) ** k):
    ...
    ok = (
        n_ok == K_ALG + 1
        and want_odd_s4(2) != jacobsthal(3)
        and want_odd_s4(7) != jacobsthal(7)
        and want_odd_s4(8) != want_clip_gp(8)
        and want_odd_s4(8) != want_even_slice(8)
        and want_odd_s4(8) != want_ug_fl(8)
        and want_odd_s4(8) != want_j0_odd_unp(8)
        and want_odd_s4(8) == 86
        and want_odd_s4(8) == jacobsthal(8) + 1
...
def odd_s4_fold() -> dict:
    """k<=8 odd partner-5U+4 slice vs J_k+(-1)^k."""
    n_ok = 0
    rows = {}
    ...
    ok = (
        ...
        and rows["2"]["n_at"] == 2
        and rows["3"]["n_at"] == 2
        and rows["3"]["first"] == [29, 14]
        and rows["4"]["first"] == [45, 6]
        and rows["7"]["n_at"] == 42
        and rows["8"]["n_at"] == 86
        and rows["8"]["first"] == [645, 6]
        and want_odd_s4(4) == 6
        and want_odd_s4(5) == 10
        and want_odd_s4(6) == 22
    )
    return {"ok": ok, "n_ok": n_ok, "k_hi": K_COUNT, "rows": rows}


def killed_eq() -> dict:
    """odd slice equals J_{k+1} at k=2; equals J_k at k=7; equals clip_gp at k=8."""
    ok = (
        want_odd_s4(2) != jacobsthal(3)
        and want_odd_s4(7) != jacobsthal(7)
        and want_odd_s4(8) != want_clip_gp(8)
        and want_odd_s4(8) != want_even_slice(8)
        and want_odd_s4(8) != want_ug_fl(8)
...
def prefixes() -> dict:
    ...
    ok = (
        ...
        and ws["verdict"]["ph4_j4_partner_5U4"] == "LEMMA"
...
def main() -> None:
    ...
    dump = {
        ...
        "odd_s4_fold": {k: cnt[k] for k in cnt if k != "ok"},
        ...
        "lemmas": {
            "odd_s4_eq_J_k_plus_m1_k_ge_2": True,
            "first_odd_s4_eq_wp_k_ge_4": True,
            "packed_R_eq_ST": False,
            "E_all_k": False,
            "prize": False,
        },
        "verdict": {
            "odd_s4_eq_J_k_plus_m1_k_ge_2": "LEMMA",
            "first_odd_s4_eq_wp_k_ge_4": "LEMMA",
            "odd_s4_j6_partner_5U4": "LEMMA",
            "odd_s4_eq_J_k1_at_k2": "KILLED",
            "odd_s4_eq_J_k_at_k7": "KILLED",
            "odd_s4_eq_clip_gp_at_k8": "KILLED",
            "odd_s4_eq_even_slice_at_k8": "KILLED",
            "odd_s4_eq_ug_at_k8": "KILLED",
            "odd_s4_eq_j0_odd_at_k8": "KILLED",
            "ph5_G6_eq_1_at_k3": "KILLED",
            "pal_c_eq_ST": "KILLED",
            "E_q10_10": "CERTIFIED",
    ...
    print("odd s4 k8", dump["odd_s4_fold"]["rows"]["8"])
