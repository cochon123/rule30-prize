...
On covering J6,J10 for k<=6, every Green one at packed p=4 has
packed AND (no silent G=1 columns). The G=1 count equals Cycle LC's
AND count want_p4_n, so the p=4 contribution to J is the Green-ones
XOR at p=4, which is 1. Not AND iff G=1 (AND still fires on G=0);
not silent-free at p=14; not silent-free at every unique p; not a
...
Run: python3 research/cycle_lv.py --certify
Dump: research/cycle_lv.json
"""
...
LU_JSON = Path(__file__).resolve().parent / "cycle_lu.json"
...
def p4_g1_and() -> dict:
    """k<=6: every G=1 column at p=4 has packed AND, count want_p4_n."""
    n_ok = n_g1 = n_p4_g1 = n_silent = 0
    rows = {}
...
def killed_iff() -> dict:
    """AND at p=4 fires iff G=1: k=0 q=10 has AND on G=0."""
    w = _walk_slot(0, 10, 4)
...
def killed_eq_j() -> dict:
    """p=4 Green-ones XOR is J: k=1 q=6 has xor_slot=1 and J=0."""
    w = _walk_slot(1, 6, 4)
    ok = w.get("ok") and w["xor_slot"] == 1 and w["xor_j"] == 0
    return {"ok": ok, "k": 1, "q": 6, "xor_slot": w["xor_slot"], "xor_j": w["xor_j"]}


def killed_all_unique() -> dict:
    """every unique p is silent-free on G=1: p=14 is not."""
    w = _walk_slot(1, 10, 14)
    ok = w.get("ok") and w["silent"] > 0
    return {"ok": ok, "p": 14, "k": 1, "q": 10, "silent": w["silent"]}


def prefixes() -> dict:
    lu = json.loads(LU_JSON.read_text())
    ok = (
        lu["checks"]["all_ok"]
        and lu["verdict"]["p114_0100"] == "LEMMA"
        and lu["verdict"]["p76_0011"] == "LEMMA"
        and lu["verdict"]["p4_1001"] == "LEMMA"
        and lu["verdict"]["prize"] == "unsolved"
    )
    return {"ok": ok}
...
def self_checks(
    ...
    assert PAT1001 == (1, 0, 0, 1)
    assert want_p4_n(0, 6) == 1
    assert want_p4_n(1, 10) == 3
    return {"all_ok": True}
...
def main() -> None:
    ...
    dump = {
        ...
        "checks": checks,
        "p4_g1_and": {k: rt[k] for k in rt if k != "ok"},
        "g4_xor_cover": {
            "n_ok": sc["n_ok"],
            "n_g1": sc["n_g1"],
        ...
        "killed_all_unique": {k: k3[k] for k in k3 if k != "ok"},
        "lemmas": {
            "p4_g1_and": True,
            "p114_0100": True,
            "p76_0011": True,
