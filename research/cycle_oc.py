#!/usr/bin/env python3
"""Cycle OC: on q=10 through k<=10, NG inner T is 1 iff k>2 and k%3==2.
...
iff k>2 and k%3==2. Prefix Cycle NG for k<=8; walk k=9 and k=10.
Companion OB outer still holds at those k. Not a death at k=9
(xor=0=want); not a death at k=10 (xor=0=want); not T (k=5: 1 vs
0); not rest (k=5: 1 vs 0); not outer T (k=2: 0 vs 1); not NN
T-cell G(j+1) (k=9: 0 vs 1); not empty (k=9 n_in=2616); not
pointwise 0 (k=9 n_ing=1038); not 0 (k=5: 1); not the form for
all k. Do not claim T is 1 iff k=2; do not claim J6=J10=0 implies
J18=1 for all k; do not push even-spine past k=18; do not bump
all n0=16 past 414990. Not a prize claim.

Run: python3 research/cycle_oc.py --certify
Dump: research/cycle_oc.json
"""
...
OB_JSON = Path(__file__).resolve().parent / "cycle_ob.json"
...
def q10_t_inner_10() -> dict:
    """q=10 k<=10: prefix NG k<=8; walk k=9,10; xor_in = want_t_inner."""
    ng = json.loads(NG_JSON.read_text())
    rows = {}
    n_ok = n_g1 = n_in = n_ing = 0
    for k in range(0, 9):
        r = ng["bothq_t_inner"]["rows"][str(k)]["j10"]
        wh = want_t_inner(k)
        if r["xor_in"] != wh:
    ...
    ok = (
        all(rows[str(k)]["xor_in"] == want_t_inner(k) for k in range(0, 11))
        and rows["2"]["n_in"] == 0
        and rows["2"]["xor_in"] == 0
        ...
        and rows["8"]["xor_in"] == 1
        and rows["9"]["xor_in"] == 0
        and rows["9"]["n_in"] == 2616
        and rows["9"]["n_ing"] == 1038
        and rows["10"]["xor_in"] == 0
        and rows["10"]["n_in"] == 8655
        and rows["10"]["n_ing"] == 3390
        and want_t_inner(9) == 0
        and want_t_inner(10) == 0
...
def killed_eq_outer(q10: dict) -> dict:
    r = q10["rows"]["2"]
...
def killed_eq_jp1(q10: dict) -> dict:
    r = q10["rows"]["9"]
    ok = r["xor_in"] == 0 and r["want_jp1"] == 1
    return {"ok": ok, "k": 9, "q": 10, "xor_in": r["xor_in"], "want_jp1": r["want_jp1"]}
...
def killed_pointwise(q10: dict) -> dict:
    r = q10["rows"]["9"]
    ok = r["n_ing"] == 1038 and r["xor_in"] == 0
    return {"ok": ok, "k": 9, "q": 10, "n_ing": r["n_ing"]}


def prefixes() -> dict:
    ng = json.loads(NG_JSON.read_text())
    ob = json.loads(OB_JSON.read_text())
    mj = json.loads(MJ_JSON.read_text())
    ok = (
        ng["checks"]["all_ok"]
        and ob["checks"]["all_ok"]
        and mj["checks"]["all_ok"]
        and ng["verdict"]["t_inner"] == "LEMMA"
        and ob["verdict"]["t_outer_10"] == "LEMMA"
        and ng["verdict"]["prize"] == "unsolved"
        and want_t_inner(8) == 1
...
def main() -> None:
    ...
    pref = prefixes()
    checks = self_checks(
        c20, q10, kd9, kd10, kt, kr, ko, kj, kz, kemp, kpw, sc, pref
    )
    dump = {
        "cycle": "OC",
        "wall_s": round(time.perf_counter() - t0, 3),
        "checks": checks,
        "q10_t_inner_10": {k: q10[k] for k in q10 if k != "ok"},
        "g4_xor_cover": {
            "n_ok": sc["n_ok"],
            "n_g1": sc["n_g1"],
    ...
    print(
        "q10_t_inner_10 n_ok",
        dump["q10_t_inner_10"]["n_ok"],
        "n_g1",
        dump["q10_t_inner_10"]["n_g1"],
        "n_in",
        dump["q10_t_inner_10"]["n_in"],
        "n_ing",
        dump["q10_t_inner_10"]["n_ing"],
    )
