"""ideas12 item 1: origin-in-hull weight-8 span-24 census.

Cycle L (research/period2_weight8.md) found L=35 on a weight-8 span-24
row, but every L>=32 placement had the origin outside the live convex
hull. The prize seed has a 1 at the origin. This script repeats that
census at tcap=32w+512, keeping only placements with
...
Run: python3 research/period2_hull.py --certify
Dump: research/period2_hull.json, research/period2_hull.md

This file does not modify period2_weight8.py.
"""
...
def run_census(log: list[str]) -> dict:
    ...
    n_tasks = len(tasks)
    log.append(
        f"census tasks={n_tasks} nproc={workers} span<= {SPAN_MAX} wt<= {WT_MAX} "
        f"placement=in-hull C in [0,s]"
    )
...
def census_membership(merged: dict | None = None) -> dict:
    """281769 is in-hull L=31; 17057305 is off-hull and must be excluded."""
    out: dict = {}
    ge30 = (merged or {}).get("ge30") or []
    ge_keys = {(r["w"], r["mask"]) for r in ge30}
    best = (merged or {}).get("best")

    for tag, live, mask, w, Lexp, must_include in (
    ...
    if merged is not None:
        out["census_includes_281769"] = (17, MASK_281769) in ge_keys or (
            best is not None
            and best.get("mask") == MASK_281769
            and best.get("w") == 17
        )
        out["census_excludes_17057305"] = (38, MASK_17057305) not in ge_keys and not (
            best is not None
            and best.get("mask") == MASK_17057305
            and best.get("w") == 38
        )
        out["ok"] = (
            out["ok"]
            and out["census_includes_281769"]
...
def write_markdown(dump: dict) -> str:
    ...
    p("# Period-2 `L_run`: origin-in-hull weight-8 span-24 census")
    p()
    p("This note extends Cycle L (`research/period2_weight8.md`). It does")
    ...
    p("`≤8` and span `≤24` with the origin in the live convex hull, not a")
    p("uniform-in-`w` bound.")
    p()
    p("Helper: `research/period2_hull.py --certify`. Dump:")
    p("`research/period2_hull.json`. Packed evolution is imported from")
    p("`research/period2_fiber.py` and that file is not modified.")
    p("`research/period2_weight8.py` is not modified.")
    p()
    p("## Attack")
    p()
    p("Cycle L’s weight-8 span-24 census at `tcap=32w+512` reached `L=35`")
    p("on mask `17057305` (true radius 38), but every `L≥32` placement had")
    p("the origin **outside** the live convex hull: all 1s strictly left of")
    p("cell 0, or all strictly right. The prize seed has a 1 at the origin,")
    p("so those off-hull bursts are not prize-shaped. ideas12 item 1 asks")
    p("for the same census restricted to placements with")
    p()
    p("```")
    ...
    p()
    p("**Kill:** an in-hull weight-`≤8` span-`≤24` row with a period-2")
    p("centre run of length `≥32` after doubling `tcap`.")
    p()
    p("**Finite theorem (this class only):** every in-hull weight-`≤8`")
    p("span-`≤24` row has `L≤31`.")
    p()
    ...
    p("  the hull census excludes this row.")
    ...
    p("  engine on both in-hull maximizers. Hull scan `nC=s+1` does not")
    p("  contain the off-hull centre of mask `17057305`.")
    p("- Random in-hull centres (`C in [0,s]`) satisfy `min≤0≤max` and match")
    p("  the packed engine; random `C` outside `[0,s]` are off-hull.")
    ...
    if known:
        p("Known rows, scored inside this in-hull class:")
        p()
        for tag in ("4369552", "281769", "17057305"):
            ...
            if tag == "17057305":
                p(
                    ...
                    f"`C={r.get('C')}` not in `[0,s]`, excluded, "
                    f"ok={r.get('ok')}"
                )
            else:
                p(
                    ...
                    f"in-hull={r.get('in_hull')}, ok={r.get('ok')}"
                )
        p()
    ...
    p(f"  `1..8` and span `≤{SPAN_MAX}` with the origin in the live convex")
    p("  hull (`min(live)≤0≤max(live)`, `s+1` translations), produces a")
    p("  longest period-2 centre run of length")
    p(f"  `L_run={dump.get('best_L')}` inside `tcap=32w+512`.")
    p("- Self-checks as above, including in-hull `w=17` mask `281769`")
    p("  `L=31`, and exclusion of off-hull `w=38` mask `17057305` (`L=35`,")
    p("  all live `< 0`).")
    ...
    p(f"- Finite theorem `L≤31` on in-hull weight≤8 span≤24: {'yes' if dump.get('finite_theorem') else 'no'}.")
    p("- Survive / uniform proof: no.")
    p()
    p("## Files")
    p()
    p("- `research/period2_hull.md` (this note)")
    p("- `research/period2_hull.py` (`--certify` runs the checks and the")
    p("  in-hull weight-8 span-24 census)")
    p("- `research/period2_hull.json` (dump)")
    p()
    return "\n".join(a) + "\n"
...
def certify() -> dict:
    ...
    log.append(
        f"census membership 281769={known.get('census_includes_281769')} "
    ...
    if found_ge32:
        verdict = "KILL"
        reason = (
            f"an in-hull weight<=8 span<=24 row has a period-2 centre run of "
            f"length {L_report}>=32 (tcap=32w+512, still holds after doubling). "
            "The census kills a bound L<=31 on origin-in-hull supports of this "
            "class. The run is a finite burst unless it reaches the cap. This "
            "does not exclude eventual period 2, and it is not a uniform-in-w "
            "theorem."
        )
    elif finite:
        ...
        reason = (
            ...
            f"(maxlive-minlive)<=24 with the origin in the live convex hull "
            ...
            f"281769 (w=17) is in-hull with L=31; mask 17057305 (w=38) is "
            f"off-hull (all live < 0) and is excluded. This is a finite "
            f"theorem for those supports only, not a uniform-in-w bound, and "
            f"not an exclusion of eventual period 2."
        )
    else:
        verdict = "L_GT_31"
        reason = (
    ...
    dump = {
        ...
        "problem": (
            ...
            "(maxlive-minlive)<=24 with origin in the live convex hull "
