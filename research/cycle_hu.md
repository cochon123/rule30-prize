...
\(G=1\) AND is **not** iff FRESH. AND is **not** only on \(G=1\).
Do **not** claim
...
Helper: `and_clause`. Certify: `python3 research/cycle_hu.py --certify` (~0.09s).
...
for \(k\le 6\) (\(n_{\mathrm{ok}}=95821\), \(n_{\mathrm{and}}=21021\) of
which \(n_{G=1}=4522\)). Every covering AND is FRESH or CONT. Odd-\(s\)
...
AND only on \(G=1\): at \(k=0\), \(s=3\), \(n=3\), \(j=2\), \(p=6\),
FRESH \(0100\), \(G(3,2)=0\).
