#!/usr/bin/env python3
"""Cycle JN: dual of pair_dbl_fives is reverse-swap of the two LIFT1 windows.

Palindrome dual j -> 2n-j-1 sends even-j 00011/01110 to odd-j
01110/11000. Dual of even-j windows is not even-j; dual of odd-j
windows is not odd-j; dual pair_dbl is the reverse-swap. Do not
...
Run: python3 research/cycle_jn.py --certify
Dump: research/cycle_jn.json
"""
...
from cycle_jl import PAIR_EVEN, PAIR_ODD, pair_dbl_fives
from period2_fiber import rule30_step

OUT = Path(__file__).resolve().with_suffix(".json")
JM_JSON = Path(__file__).resolve().parent / "cycle_jm.json"
HF_JSON = Path(__file__).resolve().parent / "cycle_hf.json"
HG_JSON = Path(__file__).resolve().parent / "cycle_hg.json"


def pair_dbl_rev(fives):
    """Reverse-swap of a pair of LIFT1 windows."""
    a, b = fives
    return (b[::-1], a[::-1])


def dbl_rev_table() -> dict:
    """n<64: dual pair_dbl_fives is reverse-swap; even maps to odd."""
    ...
    for n in range(0, 64):
        for j in range(0, 2 * n):
            ...
            f2 = pair_dbl_fives(n, j2)
            n2 = 2 * n
            fiveL2 = iso1_lift5(n2, 2 * j2)
            fiveR2 = iso1_lift5(n2, 2 * j2 + 2)
            if f2 != pair_dbl_rev(pred) or (fiveL2, fiveR2) != f2:
                return {
                    "ok": False,
                    "miss": True,
                    "n": n,
    ...
    ok = (
        ...
        and n_even == 256
        and n_odd == 256
        and pair_dbl_fives(1, 1) == pair_dbl_rev(pair_dbl_fives(1, 0))
    )
    return {
        "ok": ok,
        "n_g11": n_g11,
...
def _walk_rev(k: int, q: int) -> dict:
    """Dual-in-support pair_dbl_fives reverse-swap; packed J XOR."""
    U = 1 << k
...
def killed_even_stays() -> dict:
    """Dual of even-j windows stays even-j: G(1,0) 00011/01110 maps to 01110/11000."""
    k, s, n, j, p, p2 = 0, 3, 1, 0, 6, 4
    j2 = dual_pair_start(n, j)
    f = pair_dbl_fives(n, j)
    f2 = pair_dbl_fives(n, j2)
    ok = (
        j % 2 == 0
        and f == PAIR_EVEN
    ...
    return {
        ...
        "p": p,
        "p2": p2,
        "fives": [list(f[0]), list(f[1])],
        "fives2": [list(f2[0]), list(f2[1])],
    }


def killed_odd_stays() -> dict:
    """Dual of odd-j windows stays odd-j: G(1,1) 01110/11000 maps to 00011/01110."""
...
def prefixes() -> dict:
    jm = json.loads(JM_JSON.read_text())
    ok = (
        jm["checks"]["all_ok"]
        and jm["verdict"]["half_neigh3_dual_rev"] == "LEMMA"
        and jm["verdict"]["prize"] == "unsolved"
    )
    return {"ok": ok}
...
def self_checks(
    ...
    assert pair_dbl_fives(1, 1) == pair_dbl_rev(pair_dbl_fives(1, 0))
    return {"all_ok": True}
